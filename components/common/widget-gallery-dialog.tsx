...
const widgetMetadata: Record<string, { 
  ...
  'releaseTimeline': {
    ...
    description: 'Track product releases and milestones',
    category: 'Planning',
    color: 'border-blue-500 bg-blue-50 dark:bg-blue-950/30',
    iconBg: 'bg-blue-500/15 text-blue-600 dark:text-blue-400'
  },
  ...
  'multiProjectRoadmap': {
    ...
    description: 'View roadmap across multiple projects',
    category: 'Planning',
    color: 'border-purple-500 bg-purple-50 dark:bg-purple-950/30',
    iconBg: 'bg-purple-500/15 text-purple-600 dark:text-purple-400'
  },
  'strategicPlanning': {
    ...
    description: 'Strategic initiatives and long-term goals',
    category: 'Planning',
    color: 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/30',
    iconBg: 'bg-indigo-500/15 text-indigo-600 dark:text-indigo-400'
  },
  'riskPrediction': {
    ...
    description: 'AI-powered risk assessment and prediction',
    category: 'Analytics',
    color: 'border-orange-500 bg-orange-50 dark:bg-orange-950/30',
    iconBg: 'bg-orange-500/15 text-orange-600 dark:text-orange-400'
  },
  'capacityForecast': {
    ...
    description: 'Predict team capacity and resource needs',
    category: 'Resources',
    color: 'border-cyan-500 bg-cyan-50 dark:bg-cyan-950/30',
    iconBg: 'bg-cyan-500/15 text-cyan-600 dark:text-cyan-400'
  },
  'releaseSuccessPrediction': {
    icon: CheckCircle2,
    title: 'Release Success Prediction',
    description: 'AI prediction of release success probability',
    category: 'Analytics',
    color: 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30',
    iconBg: 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400'
  },
  'whatIfScenarios': {
    ...
    description: 'Simulate different project scenarios',
    category: 'Planning',
    color: 'border-violet-500 bg-violet-50 dark:bg-violet-950/30',
    iconBg: 'bg-violet-500/15 text-violet-600 dark:text-violet-400'
  },
  'stakeholderUpdates': {
    ...
    description: 'Communicate updates to stakeholders',
    category: 'Tools',
    color: 'border-pink-500 bg-pink-50 dark:bg-pink-950/30',
    iconBg: 'bg-pink-500/15 text-pink-600 dark:text-pink-400'
  },
  'collaborationActivity': {
    ...
    description: 'Track team collaboration and interactions',
    category: 'Analytics',
    color: 'border-teal-500 bg-teal-50 dark:bg-teal-950/30',
    iconBg: 'bg-teal-500/15 text-teal-600 dark:text-teal-400'
  },
  'feedbackIntegration': {
    ...
    description: 'Collect and integrate user feedback',
    category: 'Tools',
    color: 'border-lime-500 bg-lime-50 dark:bg-lime-950/30',
    iconBg: 'bg-lime-500/15 text-lime-600 dark:text-lime-400'
  },
...
export function WidgetGalleryDialog({ open, onOpenChange, widgets, toggleWidget }: WidgetGalleryDialogProps) {
  ...
  // Create a map of existing widgets by type for quick lookup
  const widgetsByType = new Map(widgets.map(w => [w.type, w]))

  // Get all available widgets from metadata, merging with existing widgets
  const allAvailableWidgets = Object.keys(widgetMetadata).map((type) => {
    const existingWidget = widgetsByType.get(type)
    if (existingWidget) {
      return existingWidget
    }
    // Create a new widget entry for widgets not yet in the list
    return {
      id: type,
      type: type,
      visible: false
  ...
  const visibleCount = allAvailableWidgets.filter(w => w.visible).length
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          ...
          <DialogDescription className="text-sm mt-2">
            Choose which widgets to display on your dashboard. {visibleCount} of {allAvailableWidgets.length} widgets are currently visible.
          </DialogDescription>
        </DialogHeader>
